...
# Values of taus to try
taus <- c(1:2)

results_tau <- foreach(split=rep(c(1:splits),length(taus)), tau=rep(taus, each=splits), .combine=rbind) %dopar% {
  start_time <- Sys.time()
  library(e1071)
  library(reshape2)
  ...
  for (i in unique(data_train_raw$school)){
    ...
    tunesvm_it = tune.svm(y=data_train_school_raw[,1], x=data_train_school_raw[,-c(1,2)], 
                          ...
                          cost=seq(from=0.1, to=10, length.out=10),
                          gamma=seq(from=0.01, to=1, length.out=5),
                          tunecontrol = tc)
    cv_svr_it <- rbind(cv_svr_it,data.frame(task=i, C=tunesvm_it$best.parameters$cost, 
                                            gamma=tunesvm_it$best.parameters$gamma))
  ...
  for (i in unique(data_train_raw$school)){
    ...
    cost_it <- cv_svr_it[cv_svr_it$task==i,'C']
    gamma_it <- cv_svr_it[cv_svr_it$task==i,'gamma']
    
    modelsvm_it = svm(y=data_train_school_raw[,1], x=data_train_school_raw[,-c(1,2)], type="eps-regression", 
                      kernel="radial", cost=cost_it, gamma=gamma_it)
  ...
  for (i in 1:139){
    ...
    gamma_cv <- tunesvm_svr$best.parameters$gamma
    
    # SVR training and predicting
    modelsvm_cluster = svm(y=data_train_cluster_svr[,1], x=data_train_cluster_svr[,-c(1,2)], type="eps-regression", 
                           kernel="radial", cost=cost_cv, gamma=gamma_cv)
  ...
  run_result <- data.frame(tau=tau, split=split, ev=ev_svr_final, time=end_time - start_time)
  
  write.table(run_result, file="CODES/results_19.csv", 
