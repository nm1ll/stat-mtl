...
filename <- paste(c("adjustment_",toString(lambda),"_loop",toString(loop),".RData"), collapse="")
load(filename)
...
ev_df <- data.frame(lambda=lambda, loop=loop, cost=cost, gamma=gamma, ev_additive=ev_additive)

setwd("/projects/gzhang2016227/nikolay/thesis-additivemtl-linearcomponent-svr-adjustment/")
filename <- paste("results_loop",toString(loop),".csv", sep="")  
